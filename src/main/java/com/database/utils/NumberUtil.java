...
public class NumberUtil {
     ...
     * @param @param decimal 保留小数点后几位
     * @param @return  参数说明
     * @return Double    返回类型
     * @throws
     * @author luozh
     */
    public static Double division(Object dividend, Object divisor, int decimal){
        if(dividend == null){
            dividend = "0";
        }
        if (!NumberUtils.isNumber(dividend.toString())){
            throw new NumberFormatException("数字格式化异常");
        }
        if(divisor == null){
            divisor = "0";
        }
        if (!NumberUtils.isNumber(divisor.toString())){
            throw new NumberFormatException("数字格式化异常");
        }
        double result = Double.valueOf(dividend.toString()) / Double.valueOf(divisor.toString());
